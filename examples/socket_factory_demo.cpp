...
void demonstrateBasicCreation() {
    ...
    if (tcp_result.isSuccess()) {
        ...
        std::cout << "  Family: "
                  << (socket.getAddressFamily() == AddressFamily::IPv4 ? "IPv4"
                                                                       : "IPv6")
                  << "\n";
    } else {
...
void demonstrateServerCreation() {
    ...
    if (server_result.isSuccess()) {
        ...
        if (auto endpoint = server.getLocalEndpoint()) {
            std::cout << "  Listening on: " << endpoint.value().toString()
                      << "\n";
        }
...
void demonstrateClientConnection() {
    ...
    if (server_result.isError()) {
        std::cout << "? Failed to create test server: "
    ...
    if (client_result.isSuccess()) {
        ...
        if (auto local = client.getLocalEndpoint()) {
            std::cout << "  Client local: " << local.value().toString() << "\n";
        }
        if (auto peer = client.getPeerEndpoint()) {
            std::cout << "  Client peer: " << peer.value().toString() << "\n";
        }
    } else {
...
void demonstratePortUtilities() {
    ...
    auto port_check = SocketFactory::isPortAvailable(
        AddressFamily::IPv4, "127.0.0.1", Port{8080});
    ...
    auto find_port = SocketFactory::findAvailablePort(AddressFamily::IPv4,
        "127.0.0.1", Port{49152}, // Start of ephemeral range
...
void demonstrateErrorHandling() {
    ...
    // Try to create a server on a privileged port (may fail depending on
    // permissions)
    auto privileged_port = SocketFactory::createTcpServer(
        AddressFamily::IPv4, ServerBind{"0.0.0.0", Port{80}} // HTTP port
    );
...
void demonstratePerformanceBenefits() {
    ...
    std::cout << "? Created " << iterations << " sockets in "
              << duration.count() << " ?s\n";
    std::cout << "  Average: " << duration.count() / iterations
              << " ?s per socket\n";
    std::cout
        << "  Zero error message construction overhead for success cases\n\n";
    ...
    if (error_result.isError()) {
        ...
        std::cout << "  Second access: " << error_result.message()
                  << " (cached)\n";
    }
