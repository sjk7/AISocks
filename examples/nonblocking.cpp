...
void runServerNonBlocking() {
    ...
    std::cout << "Client connected! Starting non-blocking data transfer..." << std::endl;
    ...
    while (totalSent < TOTAL_DATA) {
        ...
        int bytesSent = clientSocket->send(buffer.data() + bufferOffset, 
                                           currentChunkSize - bufferOffset);
...
void runClientNonBlocking() {
    ...
    std::cout << "Connected to server! Starting non-blocking data transfer..." << std::endl;
...
int main() {
    std::cout << "=== aiSocks Library - Non-Blocking I/O Speed Test ===" << std::endl;
