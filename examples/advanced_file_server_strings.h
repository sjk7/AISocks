...
inline const char* AUTH_SECTION
    = "\n🔐 BASIC AUTHENTICATION:\n   URL: http://localhost:8080/\n   "
      "Username: admin\n   Password: secret\n   → Browser will show login "
      "dialog\n   → Check access.log for authentication attempts\n";
inline const char* DIR_SECTION
    = "\n📁 DIRECTORY LISTING:\n   URL: http://localhost:8080/\n   → Shows "
      "enhanced directory listing with file sizes and dates\n   → Click on "
      "subdirectories to navigate\n   → Click on files to view them (browser "
      "displays or downloads based on MIME type)\n";
inline const char* FILE_SECTION
    = "\n📄 FILE SERVING:\n   URL: http://localhost:8080/index.html\n   URL: "
      "http://localhost:8080/style.css\n   URL: "
      "http://localhost:8080/script.js\n   → Files are served with correct "
...
inline const char* ERROR_SECTION
    = "\n❌ ERROR HANDLING:\n   URL: http://localhost:8080/nonexistent.html\n  "
      ...
      "normalized), 403 via curl --path-as-is\n   → curl --path-as-is "
      "http://localhost:8080/../etc/passwd -u admin:secret\n   URL: "
      "http://localhost:8080/ (with wrong auth)\n   → Custom 401 error page\n";
...
inline const char* FEATURES_SECTION
    = "\n⚙️  SERVER FEATURES:\n  - Basic authentication (admin:secret)\n  - "
      "Access logging to access.log\n  - Enhanced directory listings with file "
      "info\n  - Custom error pages with CSS styling\n  - Access control "
      "(denies .conf, .log, .tmp files)\n  - Custom MIME types for modern web "
...
inline const char* LOG_SAVED = "📝 Access log saved to: access.log\n";
inline const char* THANK_YOU = "👋 Thank you for using HttpFileServer!\n";
