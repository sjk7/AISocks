...
int main() {
    ...
    if (interfaces.empty()) {
        ...
        std::cout << "Found " << interfaces.size() << " address(es):" << std::endl;
        std::cout << std::endl;
        
        for (const auto& iface : interfaces) {
            std::cout << "Interface: " << iface.name << std::endl;
            std::cout << "  Address:   " << iface.address << std::endl;
            std::cout << "  Family:    " << (iface.family == AddressFamily::IPv4 ? "IPv4" : "IPv6") << std::endl;
    ...
    // IPv4 conversion
    uint32_t ipv4Addr = 0x7F000001;  // 127.0.0.1 in network byte order (big-endian)
