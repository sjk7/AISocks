...
int main() {
    std::cout << "=== Testing Move Semantics and Moved-From State ===" << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "  Moved to socket2, socket2 valid: " << socket2.isValid() << std::endl;
    std::cout << "  socket1 (moved-from) valid: " << socket1.isValid() << std::endl;
    
    if (!socket1.isValid()) {
        std::cout << "  ✓ PASS - socket1 properly invalidated after move" << std::endl;
    } else {
    ...
    bool bindResult = socket5.bind("127.0.0.1", 9999);
    std::cout << "    bind() returned: " << bindResult << " (expected: 0)" << std::endl;
    
    bool listenResult = socket5.listen(5);
    std::cout << "    listen() returned: " << listenResult << " (expected: 0)" << std::endl;
    
    bool connectResult = socket5.connect("127.0.0.1", 9999);
    std::cout << "    connect() returned: " << connectResult << " (expected: 0)" << std::endl;
    
    char buffer[10];
    int sendResult = socket5.send("test", 4);
    std::cout << "    send() returned: " << sendResult << " (expected: -1)" << std::endl;
    
    int recvResult = socket5.receive(buffer, sizeof(buffer));
    std::cout << "    receive() returned: " << recvResult << " (expected: -1)" << std::endl;
    
    socket5.close();  // Should be safe to call
    std::cout << "    close() executed without crash" << std::endl;
    
    SocketError error = socket5.getLastError();
    std::cout << "    getLastError() returned: " << static_cast<int>(error) << std::endl;
    
    std::string errorMsg = socket5.getErrorMessage();
    std::cout << "    getErrorMessage(): " << errorMsg << std::endl;
    
    if (bindResult == false && listenResult == false && 
        connectResult == false && sendResult == -1 && recvResult == -1) {
        std::cout << "  ✓ PASS - All operations failed gracefully" << std::endl;
    } else {
        std::cout << "  ✗ FAIL - Some operations didn't fail as expected" << std::endl;
        return 1;
    }
    std::cout << std::endl;
    ...
    std::cout << "  socket7 valid before self-assignment: " << socket7.isValid() << std::endl;
    
    socket7 = std::move(socket7);
    std::cout << "  socket7 valid after self-assignment: " << socket7.isValid() << std::endl;
    ...
    std::cout << "    getAddressFamily(): " 
              << (socket9.getAddressFamily() == AddressFamily::IPv4 ? "IPv4" : "IPv6") 
              << std::endl;
    
    std::cout << "  ✓ PASS - Moved-from state has safe default values" << std::endl;
    std::cout << std::endl;
