...
int main() {
    ...
    if (!socket.isValid()) {
        std::cerr << "Failed to create socket: " << socket.getErrorMessage() << std::endl;
        return 1;
    }
    ...
    std::cout << "  Socket is blocking: " << (defaultBlocking ? "YES" : "NO") << std::endl;
    if (defaultBlocking) {
    ...
    std::cout << "  Socket is non-blocking: " << (isNonBlocking ? "YES" : "NO") << std::endl;
    if (isNonBlocking) {
        std::cout << "  ✓ PASS - Successfully set to non-blocking" << std::endl;
    } else {
        std::cout << "  ✗ FAIL - State not updated after setBlocking(false)" << std::endl;
        return 1;
    }
    std::cout << std::endl;
    ...
    std::cout << "  Socket is blocking: " << (isBlockingAgain ? "YES" : "NO") << std::endl;
    if (isBlockingAgain) {
        std::cout << "  ✓ PASS - Successfully set back to blocking" << std::endl;
    } else {
        std::cout << "  ✗ FAIL - State not updated after setBlocking(true)" << std::endl;
        return 1;
    }
    std::cout << std::endl;
    ...
    for (int i = 0; i < 5; i++) {
        ...
        std::cout << "  Toggle " << (i+1) << ": Expected=" 
                  << (targetState ? "blocking" : "non-blocking")
                  << ", Actual=" << (currentState ? "blocking" : "non-blocking");
    ...
    if (allPassed) {
        ...
        std::cout << "  ✗ FAIL - Some toggles not tracked correctly" << std::endl;
        return 1;
    }
    std::cout << std::endl;
    ...
    if (serverSocket.bind("127.0.0.1", 9999) && serverSocket.listen(1)) {
        ...
        if (acceptedSocket) {
            ...
            if (acceptedIsBlocking) {
                std::cout << "  ✓ PASS - Accepted socket defaults to blocking" << std::endl;
            } else {
                std::cout << "  ✗ FAIL - Accepted socket should default to blocking" << std::endl;
                clientThread.join();
                return 1;
            }
        ...
        std::cout << "  ⚠ SKIP - Could not bind/listen on port 9999" << std::endl;
    }
    std::cout << std::endl;
