...
echo "=== Performance Comparison: aiSocksEchoServer vs nginx ==="
echo "Each server tested 5 times with curl"
echo ""

# Test aiSocksEchoServer
echo "Testing aiSocksEchoServer..."
aiSocks_times=()
for i in {1..5}; do
    start_time=$(date +%s%N)
...
if [ $aiSocks_avg -lt $nginx_avg ]; then
    ...
    echo "🎉 aiSocksEchoServer is ${improvement}% faster than nginx!"
else
    slower=$(( (aiSocks_avg * 100) / nginx_avg - 100 ))
    echo "nginx is ${slower}% faster than aiSocksEchoServer"
fi
