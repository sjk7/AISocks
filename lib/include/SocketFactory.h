...
public:
    ...
    static Result<TcpSocket> createTcpSocket(AddressFamily family = AddressFamily::IPv4);
    
    // Create a basic UDP socket (not bound)
    static Result<UdpSocket> createUdpSocket(AddressFamily family = AddressFamily::IPv4);
    ...
    static Result<Port> findAvailablePort(
        AddressFamily family,
        const std::string& address,
...
private:
    ...
    static Result<TcpSocket> listenSocket(
        TcpSocket&& socket,
        int backlog
    );
