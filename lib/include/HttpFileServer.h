...
class HttpFileServer : public HttpPollServer {
    ...
    void buildResponse(HttpClientState& state) override {
        ...
        // ═══════════════════════════════════════════════════════════════════════
        // CRITICAL SECURITY CHECK: PATH TRAVERSAL PREVENTION VIA
        // CANONICALIZATION
        // ═══════════════════════════════════════════════════════════════════════
        //
        // WHAT WE'RE PROTECTING AGAINST:
        // Path traversal attacks attempt to access files outside the document
        // root by using ".." (parent directory) components in the URL path.
        //
        // ATTACK EXAMPLES:
        //   GET /../../../etc/passwd
        ...
        // 3. Without canonicalization, simple string check would see "www/"
        // prefix
    ...
    virtual FileInfo getFileInfo(const std::string& filePath) const {
        ...
        if (!info.isDirectory) {
            ...
            if (config_.enableETag) {
                ...
                etag.appendFormat("\"%zu-%ld\"", info.size,
    ...
    virtual void handleFileRequest(HttpClientState& state,
        ...
        bool useCache = config_.enableCache
        ...
        // TOCTOU-safe file handling: open file once, lock it, check descriptor,
        // read from same descriptor
        File file(filePath.c_str(), "rb");
        if (!file.isOpen()) {
            sendError(
                state, 500, "Internal Server Error", "Failed to open file");
            return;
        }
        ...
        // Check conditional headers using original fileInfo (from path-based
        // check)
        ...
        StringBuilder response(
            512 + fileContent.size()); // Reserve for headers + content
    ...
    void sendCachedFile(HttpClientState& state, const std::string& filePath,
        ...
        StringBuilder response(
            512 + cached.size); // Reserve for headers + cached content
        response.append("HTTP/1.1 200 OK\r\n");
        response.append("Content-Type: ");
        response.append(getMimeType(filePath));
        if (getMimeType(filePath).find("text/") == 0
