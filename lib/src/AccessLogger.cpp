...
int AccessLogger::extractStatusCode(std::string_view dataBuf) {
    // Expected: "HTTP/1.x NNN ..."
    const size_t sp1 = dataBuf.find(' ');
    if (sp1 == std::string::npos || sp1 + 1 >= dataBuf.size()) return 0;
    const char* p = dataBuf.data() + sp1 + 1;
    char* end = nullptr;
