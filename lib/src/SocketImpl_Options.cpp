...
bool SocketImpl::setBlocking(bool blocking) {
    ...
    if (ioctlsocket(socketHandle, FIONBIO, &mode) != 0) { setError(SocketError::SetOptionFailed, "Failed to set blocking mode"); return false; }
#else
    int flags = fcntl(socketHandle, F_GETFL, 0);
    if (flags == -1) { setError(SocketError::SetOptionFailed, "Failed to get socket flags"); return false; }
    flags = blocking ? (flags & ~O_NONBLOCK) : (flags | O_NONBLOCK);
    if (fcntl(socketHandle, F_SETFL, flags) == -1) { setError(SocketError::SetOptionFailed, "Failed to set blocking mode"); return false; }
#endif
...
bool SocketImpl::setReuseAddress(bool reuse) {
    ...
    if (!reuse) {
        int exclusive = 1;
        return setSocketOption(socketHandle, SOL_SOCKET, SO_EXCLUSIVEADDRUSE, exclusive, "Failed to set exclusive address use");
    }
    return true; 
#else
...
bool SocketImpl::setTimeout(Milliseconds timeout) {
    RETURN_IF_INVALID();
    return setSocketOptionTimeout(socketHandle, SO_RCVTIMEO, std::chrono::milliseconds(timeout.count), "Failed to set receive timeout");
}
...
bool SocketImpl::setSendTimeout(Milliseconds timeout) {
    RETURN_IF_INVALID();
    return setSocketOptionTimeout(socketHandle, SO_SNDTIMEO, std::chrono::milliseconds(timeout.count), "Failed to set send timeout");
}
...
bool SocketImpl::setLingerAbort(bool enable) {
    ...
    lg.l_linger = 0; 
    if (setsockopt(socketHandle, SOL_SOCKET, SO_LINGER, reinterpret_cast<const char*>(&lg), static_cast<socklen_t>(sizeof(lg))) == SOCKET_ERROR_CODE) {
...
#else
    (void)enable;
    setError(SocketError::SetOptionFailed, "SO_REUSEPORT is not supported on this platform");
    return false;
#endif
}
...
bool SocketImpl::shutdown(ShutdownHow how) {
    RETURN_IF_INVALID();
#ifdef _WIN32
    int how_ = (how == ShutdownHow::Read) ? SD_RECEIVE : (how == ShutdownHow::Write) ? SD_SEND : SD_BOTH;
#else
    int how_ = (how == ShutdownHow::Read) ? SHUT_RD : (how == ShutdownHow::Write) ? SHUT_WR : SHUT_RDWR;
#endif
    if (::shutdown(socketHandle, how_) == SOCKET_ERROR_CODE) { setError(SocketError::Unknown, "shutdown() failed"); return false; }
