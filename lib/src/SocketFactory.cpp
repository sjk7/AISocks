...
Result<void> SocketFactory::checkSocketError(const Socket& socket, const char* operation) {
    if (!socket.isValid()) {
...
template<typename SocketType>
Result<SocketType> SocketFactory::bindSocket(SocketType&& socket, const ServerBind& config) {
    // Check socket validity first
    auto valid_check = checkSocketError(socket, "bind()");
    if (valid_check.isError()) {
        return Result<SocketType>::failure(
            valid_check.error(),
            valid_check.message().c_str(),
            0,
            false
        );
    }
...
Result<TcpSocket> SocketFactory::connectSocket(TcpSocket&& socket, const ConnectArgs& config) {
    // Check socket validity first
...
Result<UdpSocket> SocketFactory::createUdpServer(AddressFamily family, const ServerBind& config) {
...
Result<bool> SocketFactory::isPortAvailable(AddressFamily family, const std::string& address, Port port) {
    // Try to create a TCP socket and bind to the port
    auto socket_result = createTcpSocketRaw(family);
    if (socket_result.isError()) {
        return Result<bool>::failure(
            socket_result.error(),
            socket_result.message().c_str(),
            0,
            false
        );
    }
    ...
    // Try to bind
    ServerBind bind_config{address, port, 1, false};  // Minimal backlog, no reuse
    auto bind_result = bindSocket(std::move(socket_result.value()), bind_config);
...
Result<Port> SocketFactory::findAvailablePort(AddressFamily family, const std::string& address, Port startPort, Port endPort) {
    ...
    for (uint16_t port = startPort.value(); port <= endPort.value(); ++port) {
        ...
        if (available_result.isError()) {
            return Result<Port>::failure(
                available_result.error(),
                available_result.message().c_str(),
                0,
                false
            );
        }
    ...
    return Result<Port>::failure(
        SocketError::Unknown,
        "findAvailablePort: no available ports in range",
        0,
        false
    );
}
