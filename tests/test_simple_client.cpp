...
int main() {
        ...
        try {
            SimpleClient client(
                ConnectArgs{"127.0.0.1", Port{BASE}}, [&](TcpSocket& sock) {
                    ...
                    callback_called
                        = (n > 0 && std::string(buf, n) == "Hello echo");
                });
    ...
    BEGIN_TEST("SimpleClient: throws on connection failure");
    {
        ...
        REQUIRE_MSG(
            exception_caught, "SocketException thrown on connection failure");
        REQUIRE_MSG(
            !callback_called, "callback not invoked on connection failure");
    }
        ...
        try {
            SimpleClient client(
                ConnectArgs{"127.0.0.1", Port{BASE + 1}}, [](TcpSocket& sock) {
                    ...
                    REQUIRE_MSG(ep.has_value(),
                        "getPeerEndpoint returns valid endpoint");
                });
