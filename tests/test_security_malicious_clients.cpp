...
void testPathTraversalAttacks() {
    ...
    {
        std::string request = makeRequest("GET", "/%2e%2e/secret/passwords.txt");
        HttpClientState state;
        state.request = request;
    ...
    {
        std::string request = makeRequest("GET", "/%252e%252e/secret/passwords.txt");
        HttpClientState state;
        state.request = request;
...
void testXSSAttacks() {
    ...
    // Attack 1: XSS in directory listing via malicious filename
    {
        ...
        SecurityTestFramework::assert_not_contains(body, "<script>alert(1)</script>",
    ...
    {
        std::string request = makeRequest("GET", "/<script>alert(document.cookie)</script>");
        HttpClientState state;
        state.request = request;
        ...
        SecurityTestFramework::assert_not_contains(body, "<script>alert(document.cookie)</script>",
...
void testSecurityHeaders() {
    ...
    SecurityTestFramework::assert_contains(response, "X-Content-Type-Options: nosniff",
    ...
    SecurityTestFramework::assert_contains(response, "Referrer-Policy: no-referrer",
...
void testInformationLeakage() {
    ...
    // Test error page doesn't leak server version
    {
        ...
        SecurityTestFramework::assert_not_contains(body, "aiSocks HttpFileServer",
    ...
    {
        config.enableDirectoryListing = true;
        TestableHttpFileServer server2(ServerBind{"127.0.0.1", Port{0}}, config);
...
void testCacheDoSProtection() {
    ...
    // Test FileCache limits
    {
        ...
        SecurityTestFramework::assert_true(cache.size() <= cacheConfig.maxEntries,
            "Cache should not exceed maxEntries limit");
        SecurityTestFramework::assert_true(cache.totalBytes() <= cacheConfig.maxTotalBytes,
    ...
    // Test LRU eviction
    {
        ...
        SecurityTestFramework::assert_true(cache.size() == 3,
            "LRU eviction should maintain cache size limit");
...
void testURLDecodingIssues() {
    ...
    // Test that + is NOT treated as space in paths (RFC 3986)
    {
        ...
        SecurityTestFramework::assert_true(status.find("200") != std::string::npos,
...
void testMethodValidation() {
    ...
    const char* methods[] = {"POST", "PUT", "DELETE", "PATCH", "OPTIONS", "TRACE"};
    ...
    for (const char* method : methods) {
        ...
        std::string testMsg = std::string("HTTP method ") + method + " should be rejected";
        SecurityTestFramework::assert_true(status.find("405") != std::string::npos,
            testMsg);
    }
}
...
void testFileTypeRestrictions() {
    ...
    // Test that large files are rejected
    {
        ...
        SecurityTestFramework::assert_true(status.find("403") != std::string::npos,
