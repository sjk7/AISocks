...
int main() {
    ...
    BEGIN_TEST("DNS gate: queued calls consume timeout while waiting");
    {
        ...
        Socket::setDnsTestDelayForTesting(Milliseconds{180});
        ...
        waitForActiveDnsWorkers_(limit, 300ms);
        REQUIRE(Socket::activeDnsWorkersForTesting() == limit);

        auto queued = TcpSocket::createRaw();
        const auto t0 = steady_clock::now();
        bool ok = queued.connect(
            "this.host.does.not.exist.invalid", Port{80}, Milliseconds{60});
        const auto elapsed
