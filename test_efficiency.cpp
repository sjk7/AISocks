...
int main() {
    ...
    std::cout << "   Request built in " << duration.count()
              << " microseconds\n";
    ...
    std::cout << "   POST request built in " << duration.count()
              << " microseconds\n";
    std::cout << "   Request size: " << postRequest.size() << " bytes\n";
    std::cout << "   Contains JSON body: "
              << (postRequest.find("John") != std::string::npos ? "YES" : "NO")
              << "\n\n";
    ...
    for (int i = 0; i < 1000; ++i) {
        std::string testUrl = "http://test" + std::to_string(i)
            + ".example.com/api/endpoint/" + std::to_string(i);
    ...
    std::cout << "   1000 requests built in " << duration.count()
              << " milliseconds\n";
    std::cout << "   Average: "
    ...
    std::cout
        << "- std::string with reserve() for efficient request building\n";
